...
def init_db():
    ...
    # return
    db.drop_tables(
        [GroupUser, User, Role, Group, Vul, AuthMode, SystemSettings, Asset, App, VulLog, Message, MessagePoint,
         Article, Category, Extension, ExtensionLog, CronTab, CronTabLog])
    db.create_tables(
        [Role, User, Group, GroupUser, Vul, AuthMode, SystemSettings, Asset, App, VulLog, Message, MessagePoint,
         Article, Category, Extension, ExtensionLog, CronTab, CronTabLog])
    ...
    email_setting = {"smtp_host": "", "smtp_port": "", "smtp_user": "", "smtp_pass": "", "smtp_head": "",
                     "smtp_sign": "", "mail_list": ""}
    point_setting = {"ti_level_point": 1, "times_level_point": 1, "one_level_point": 1, "two_level_point": 1,
                     "three_level_point": 1, "other_level_point": 1}
    SystemSettings(global_setting=json.dumps(global_setting), point_setting=json.dumps(point_setting),
                   **email_setting).save()
...
def version_upgrade():
    ...
    conn = pymysql.connect(host=__conf__.DB_HOST, port=__conf__.DB_PORT, user=__conf__.DB_USER, passwd=__conf__.DB_PASS,
...
"""
    for item in cs:
        crontab += "\n{} root cd {};python run.py --crontab_id={} --config=dev_settings.py".format(item.crontab, path,
                                                                                                   item._id)
